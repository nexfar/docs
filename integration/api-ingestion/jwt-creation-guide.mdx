...
| **Payload** | `id`, `role`, `sessionId` | `client_id`, `environment`, `scopes`, `iat`, `exp` |
...
```json
{
  "client_id": "cli_sandbox_test_123",
  "environment": "sandbox",
  "scopes": ["ingest:write", "audit:read"],
  "iat": 1699459200,
  "exp": 1707321600,
...
Sempre gere um `client_id` único para cada cliente:
...
**URL Base:** https://api-sandbox.nexfar.com.br
**Client ID:** cli_abc123xyz789
...
```bash
# Testar token Sandbox
curl -X GET https://api-sandbox.nexfar.com.br/health \
...
<Accordion title="❌ Client ID Não Reconhecido">
  **Causa:** `client_id` no token não existe no sistema ou está incorreto.

  **Solução:**
  1. Verifique que `client_id` segue formato: `cli_XXXXXXXXX`
  2. Confirme que Client ID foi registrado corretamente
  3. Use Client ID consistente em todos os tokens desse cliente

  **Nota:** `client_id` é usado para particionar dados - deve ser único por cliente.
</Accordion>
...
|-------|-----------|---------|
| `client_id` | Identificador do cliente | `cli_abc123xyz789` |

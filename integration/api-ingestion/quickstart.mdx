...
<Steps>
  <Step title="Receber e Validar Credenciais">
    ...
    **Salve seu token em variável de ambiente:**
    ...
    <Tip>
      **Validação Opcional:** Cole seu token em [jwt.io](https://jwt.io) para inspecionar claims (`tenant_id`, `environment`, `scopes`).
    </Tip>
  ...
  <Step title="Testar Conectividade com Health Check">
    ...
    ```bash
    curl https://ingestion.nexfar.com.br/api/health
    ```
  ...
  <Step title="Enviar Primeira Requisição (Cliente)">
    ...
    curl -X POST https://ingestion.nexfar.com.br/api/v1/clients \
      ...
      -d '{
        "name": "Farmácia Quick Start",
        "cnpj": "12345678000190",
        "externalId": "CLI-QS-001",
        "razaoSocial": "Farmácia Quick Start LTDA",
        "state": "SP",
        "emailErp": "contato@quickstart.com.br",
        "phone": "11987654321",
        "drugstore": true
      }'
    ```
  ...
  <Step title="Consultar Histórico no Audit Log">
    ...
    ```bash
    curl -X GET "https://ingestion.nexfar.com.br/api/v1/audit-log?date_from=today" \
      -H "Authorization: Bearer $NEXFAR_JWT"
    ```
...
<Step title="Criar Nova Requisição">
  ...
  3. URL: `https://ingestion.nexfar.com.br/api/v1/clients`
</Step>
...
<Accordion title="❌ 400 Bad Request - Payload inválido">
  ...
  **Soluções:**
  ...
  4. Consultar schema completo na [API Reference](/api-reference)
  ...
  **Exemplo de erro:**
  ...
  <Tip>
    Use a [API Reference interativa](/api-reference) para ver schema completo de cada modelo e testar payloads.
  </Tip>
</Accordion>

<Accordion title="❌ 429 Too Many Requests - Rate limiting">
  **Causa:** Você excedeu o limite de 100 requisições por minuto por tenant_id.
...
<Card title="Explorar Outros Modelos" icon="box" href="/api-reference">
